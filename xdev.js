...
offline = false
targetpc = '0'
ownerNumber = ["18295176783@s.whatsapp.net"]
fake = '𝑪𝑹𝑬𝑨𝑻𝑶𝑹 𝑩𝑶𝑻\n𝐁𝐲 𝐉𝐚𝐜𝐤𝐒𝐢𝐭𝐨98'
fake1 = '𝐁𝐲 𝐉𝐚𝐜𝐤𝐒𝐢𝐭𝐨98'
numbernye = '0'
waktu = '-'
...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        //MESS
		mess = {
			wait: '_Eperate Mmgvo😾_',
			admin: '_Solo para administradores_',
			owner: '_Este comando solo lo puede usar Jacksito_',
			success: 'Yeiii!',
			wrongFormat: 'Formato incorrecto intentalo de nuevo',
			error: {
				stick: 'no esa pegatina:v',
				Iv: 'El enlace Dio error:v'
			},
			only: {
...
const linkwa = 'https://chat.whatsapp.com/'
    ...
    reply(`「 *OFICIAL DE LIMPIEZA* 」\n\nMuy chistoso mandando binarios? maldito sapo \nLo siento, pero tengo que sacarte ....\nAdios~`)
...
reply(`El bot no ha sido prohibido en este grupo.`)
}
}
...
gbutsan = [
  ...
  {buttonId: 'JAMUX', buttonText: {displayText: 'JAMUX'}, type: 1}
]
 gbuttonan = {
...
case 'antilink':
					...
					if (!isGroupAdmins) return reply('Solo lo pueden usar los admins')
					if (!isBotGroupAdmins) return reply('tienes que hacer al bot administrador primero')
					if (args.length < 1) return reply(`para activar el tipo: ${prefix}antilink 1`)
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Ya esta activo')
						antilink.push(from)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('_éxito al activar el antienlace en este grupo!_')
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('Sudah Mati Kak')
						var ini = antilink.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('_éxito al desactivar el antienlace en este grupo!_')
					} else {
...
if (!isGroupAdmins) return reply('Solo lo pueden usar los admins')
if (!isBotGroupAdmins) return reply('tienes que hacer al bot administrador primero')
if (args.length < 1) return reply('tipo 1 para activar\nescriba 0 para deshabilitar')
if (args[0] === "1") {
if (AntiVirtex) return reply('Activado')
antivirtex.push(from)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(antivirtex))
reply('_Éxito en la activación de antivirtex en este grupo_')
} else if (args[0] === "0") {
if (!AntiVirtex) return reply('Sudah Mati')
let off = antivirtex.indexOf(from)
antivirtex.splice(off, 1)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(antivirtex))
reply('_éxito al desactivar el antivirtex en este grupo_')
} else {
reply('1 para activar, 0 para desactivar')
}
break
case 'add':
					...
					break
					case 'kick':
if (!isOwner && !dev.key.fromMe && !isGroupAdmins) return reply('Solo administrador de grupo')
if (!isBotGroupAdmins) return reply('Dame admin gaii!')
if (!isGroup) return
...
case 'linkgroup':
if (!isGroup) return
if (!isBotGroupAdmins) return reply('Dame admin primero gai')
...
teks = `Miembro @${mentionede.split('@')[0]} Exito`
...
xdev.sendMessage(from,'Aqui JackMods equipo de desarollo',text,{quoted: hehe})
break
case 'style':
...
anu = await fetchJson(`https://api-anoncybfakeplayer.herokuapp.com/pastebin?text=${q}`, {method: 'get'})
    ...
    case 'swm':
            ...
            reply(`Envía una imagen con un${prefix}swm teks|etiquetas de texto o imagen que ya se han enviado`)
            }
            break
    case 'upswteks':
            if (!dev.key.fromMe && !isOwner) return reply(mess.owner)
	...
	case 'self':
      ...
      //********** ADD **********//
		...
		if (!dev.key.fromMe && !isOwner) return reply (mess.owner)
				...
				xdev.sendMessage(from, `Adición de audio con éxito\nCorregido por ${prefix}liststik`, MessageType.text, { quoted: dev})
				break
					case 'dellstik':
				...
				xdev.sendMessage(from, `Adición de audio con éxito\nCorregido por ${prefix}listimg`, MessageType.text, { quoted: dev})
				break
				...
				xdev.sendMessage(from, `Adición de audio con éxito\nCorregido por ${prefix}listvideo`, MessageType.text, { quoted: dev })
				break
				...
				xdev.sendMessage(from, `Adición de audio con éxito\nCorregido por${prefix}listvn`, MessageType.text, { quoted: dev})
				break
...
break
	...
	case 'play':
			if (args.length === 0) return reply(`Enviar pedidos *${prefix}play* _El título de la canción para buscar_`)
            var srch = args.join('')
...
xdev.updatePresence(from, Presence.composing)
					...
					if (isMedia && !dev.message.videoMessage || isQuotedImage) {
						...
						reply(`Exito al broadcast ${body.slice(4)}`)
						...
						reply(`Exito al broadcast:\n${body.slice(4)}`)

					}

					break
        case 'video':
            if (args.length === 0) return reply(`Enviar pedidos  *${prefix}video* _El título de la canción para buscar_`)
            var srch = args.join('')
            aramas = await yts(srch);
            aramat = aramas.all 
            var mulaikah = aramat[0].url                            
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
            ...
            case 'setppbot':
			    xdev.updatePresence(from, Presence.composing)
					if (!isOwner) return reply3('Si no eres el dueño, no tienes que actuar mucho')
					xdev.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return reply(`Envía una imagen con un ${prefix}setbotpp o imágenes etiquetadas que se han enviado`)
...
bancht.push(from)
fs.writeFileSync('./database/banchat.json', JSON.stringify(bancht))
reply(`Bot Ban exitoso en este grupo`)
break
case 'infoall': case 'tagall':
if (!isGroupAdmins && !itsMe) return reply(mess.only.admin)
if(!q) return reply('Escribe un texto aparte para el tagall, ejemplo !tagall Revivan')
if (!isGroup) return reply(mess.only.group)
...
xdev.sendMessage(from,'Este es el creador del Bot;33',text,{quoted: hehe})
}
if (button == 'INSTAGRAM') {
console.log('INSTAGRAM')
xdev.sendMessage(from, `*Sigueme en instagram🙇🏾‍♂️🙇🏾‍♂️*\n*Me inspirara a seguir sacando Nuevas Versiones*\n*y a mantener el bot activo🤒*\n\nhttps://www.instagram.com/jack_paymon/?hl=es`, text, {quoted: dev})
}
if (button == 'JAMUX') {
console.log('JAMUX')
xdev.sendMessage(from, `*Excelente aplicacion u herramienta para aprender a ser kuaker :0 y mas cosas*\n\nhttps://www.mediafire.com/download/zfwrc70zztpqu5r\n\n*cuenta con todos los buzones, disponibles, Metodos y mas!! Descargala :D 🙏🏻*`, text, {"contextInfo": {text: 'HelloWorld',"forwardingScore": 3,isForwarded: true,sendEphemeral: true,mentionedJid: [sender],"externalAdReply": {"title": `JAMUX ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": `https://telegra.ph/file/a082de791ef8aff96ec24.jpg`,"thumbnail": "","sourceUrl": ""}}, quoted: dev})
}
if (budy.startsWith('$')){
if (!dev.key.fromMe && !isOwner) return
qur = budy.slice(2)
